...
/** In-memory match: normalized exact → then slug (last non-empty path segment). */
function buildMatchIndex(seoUrls: UrlMatchInput[]) {
  const byNorm = new Map<string, UrlMatchInput[]>();
  const bySlug = new Map<string, UrlMatchInput[]>();
  for (const u of seoUrls) {
    if (u.normalized) {
      const arr = byNorm.get(u.normalized) ?? [];
      arr.push(u);
      byNorm.set(u.normalized, arr);
    }
...
function matchOne(
  norm: string | null,
  idx: ReturnType<typeof buildMatchIndex>,
  ...
  if (exact && exact.length) {
    const uniqueGroups = new Set(exact.map((u) => `${u.folder}::${u.group}`));
    return {
      matched_url_id: exact[0].url,
      matched_group_id: exact[0].group,
      match_status: uniqueGroups.size > 1 ? "group_conflict" : "matched",
    };
  }
  ...
  if (slug) {
    ...
    if (bySlug && bySlug.length === 1) {
      return {
        matched_url_id: bySlug[0].url,
        matched_group_id: bySlug[0].group,
        match_status: "matched_by_slug",
      };
    }
    if (bySlug && bySlug.length > 1) {
